...
mod.extend = function() {
    ...
    Object.defineProperties(Room.prototype, {
        ...
        'resourcesTombstone': {
            configurable: true,
            get() {
                if (_.isUndefined(this._resourcesTombstone)) {
                    
                    let tombstone = this.find(FIND_TOMBSTONES);
                    this._resourcesTombstone = {};

                    if (!_.isUndefined(this.tombstone)) {
                        Object.keys(this.tombstone.store).forEach(content => {
                            if (_.isUndefined(this._resourcesTombstone[content]))
                                this._resourcesTombstone[content] = this.tombstone.store[content];
                        });
                    }
                }
                return this._resourcesTombstone;
            }
        },
        'resourcesTerminal': {
            configurable: true,
            ...
            get() {
                if (_.isUndefined(this._resourcesReactions)) {
                    ...
                    if (!_.isUndefined(data) && !_.isUndefined(data.reactions) && !_.isUndefined(data.reactions.orders) && data.reactions.orders.length === 1) {
                        ...
                        this._resourcesReactions[ingredientA] = Math.abs(amount - mineralAmountA);
                        this._resourcesReactions[ingredientB] = Math.abs(amount - mineralAmountB);
