...
mod.boostAllocation = function () {
	...
	let roomFound,
		...
		// TODO && global.MAKE_COMPOUNDS is necessary? deleted
		// TODO global.CHECK_ORDERS_INTERVAL + 5 is ok?
		timing = (!roomTrading.boostAllocation && Memory.boostTiming.timeStamp + global.CHECK_ORDERS_INTERVAL === Game.time)
			|| Game.time % global.ALLOCATE_COMPOUNDS_INTERVAL === 0 || Memory.allocateProperties.urgentAllocate.allocate;

	// console.log(`allocate timing: ${timing}`);
	// console.log(`Game.time: ${Game.time} timeStamp: ${Memory.boostTiming.timeStamp + global.CHECK_ORDERS_INTERVAL}`);
	// console.log(`ALLOCATE_COMPOUNDS_INTERVAL: ${Game.time % global.ALLOCATE_COMPOUNDS_INTERVAL}`);
	...
	if (timing) {
		...
		for (let room of myRooms) {
			...
			for (let room of acceptedRooms) {
				...
				for (let [idx, order] of data.storage[0].orders.entries()) {
					if (order.storeAmount === 0 && order.orderRemaining <= 0) {
						data.storage[0].orders.splice(idx, 1);
						idx--;
					}
				}
			}
			...
			for (let compound in Memory.compoundsToAllocate) {
				...
				if (compoundObject.allocate && roomEnabled) {
					...
					let storageLabExist = storageLab(room, compound),
						...
						allRoomResources = function (compound, currentRoom) {
							...
							// return returnValue;
							if (!_.isUndefined(Memory.compoundsToAllocate[compound])) {
								return currentRoom.resourcesAllButMe(compound);
							}
					...
					switch (compoundObject.storeTo) {
						...
						case 'lab':
							...
							if (!storageLabExist && !superiorAllocated && !superiorMaking) {
								...
								if (labArrayData.length > 0) {
									...
									console.log(`${room.name} has no candidates`);
							...
							if (storageLab !== null && storageLab !== undefined) {
								...
								if (roomOrderNeeded && roomOrderGranted && !superiorAllocated && !superiorMaking && numberOfOrderingRooms === 0) {
									if (_.isUndefined(roomFound) && (!room.nuked || (room.nuked && compound === 'XLH2O'))) {
										let ret = room.placeRoomOrder(storageLabId, compound, orderAmount);
										if (ret === OK) {
						...
						case 'storage':
							...
							if (roomOrderGranted && roomOrderNeeded && numberOfOrderingRooms === 0)
								global.logSystem(room.name, `compound: ${compound}`);
							...
							if (compound === 'power') {
								...
								if (_.isUndefined(roomFound) && (!room.nuked || (room.nuked && compound === 'XLH2O'))) {
									...
									if (!orderExist) {
										let ret = room.placeRoomOrder(room.storage.id, compound, amountToOrder);
										if (ret === OK) {
											global.logSystem(room.name, `${room.name} placeOrder to STORAGE ${room.storage.id}, ${amountToOrder} ${compound}`);
											global.logSystem(room.name, `have ${room.resourcesAll[compound] || 0}`);
											room.placeOrder(room.storage.id, compound, amountToOrder);
											roomFound = room;
										}
									}

								} else if (!room.nuked && roomFound)
									global.logSystem(room.name, `${room.name} placeOrder to STORAGE is delayed, ${room.storage.id}, ${compoundObject.amount} ${compound} there was already a room order in this round to ${roomFound.name}`);
								else if (room.nuked)
