	...
	if (creep.data.body.heal !== undefined) {
		...
		if (mustHealSelf || creep.hits < creep.hitsMax) {
			...
			if (mustHealSelf || !creep.attacking) {
				...
				global.logSystem(creep.room.name, `HEALING MYSELF: ${creep.name} ret: ${ret}`);
				return ret;
			}
		}
		// Heal other
			...
			if (injured.length > 0) {
				...
				if (canHeal && (shouldHeal || !creep.attacking)) {
					...
					global.logSystem(creep.room.name, `HEALING ANOTHER CREEP: ${creep.name} heals ${target.name} ret: ${ret}`);
					return ret;
				} else if (shouldHeal && !(creep.attackingRanged || creep.attacking || mustHealSelf)) {
					let ret = creep.rangedHeal(target);
					global.logSystem(creep.room.name, `RANGED HEALING ANOTHER CREEP: ${creep.name} heals ${target.name} ret: ${ret}`);
					return ret;
				}
			}
		}
