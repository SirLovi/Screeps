...
mod.newTargetNuker = function (creep) {
	let room = creep.room;
	if(!creep.room.nuker) return null;
	...
	if (amount > 0) {
		...
		if (global.DEBUG && global.TRACE)
			global.trace('Action', {actionName: 'reallocating', roomName: room.name, creepName: creep.name, structureId: nuker.id, resourceType: RESOURCE_ENERGY, needs: amount});
		if (room.storage && room.storage.charge > 0.5) {
			if (global.DEBUG && global.TRACE)
				global.trace('Action', {actionName: 'reallocating', roomName: room.name, creepName: creep.name, targetStructureId: room.storage.id, resourceType: RESOURCE_ENERGY, targetNeeds: room.storage.store[RESOURCE_ENERGY]});
			creep.data.reallocating = RESOURCE_ENERGY;
			return room.storage;
		}
		if (room.terminal && room.terminal.getNeeds(RESOURCE_ENERGY) < 0) {
			if (global.DEBUG && global.TRACE)
				global.trace('Action', {actionName: 'reallocating', roomName: room.name, creepName: creep.name, targetStructureId: room.terminal.id, resourceType: RESOURCE_ENERGY, targetNeeds: room.terminal.store[RESOURCE_ENERGY]});
			creep.data.reallocating = RESOURCE_ENERGY;
	...
	if (amount > 0) {
		...
		if (global.DEBUG && global.TRACE)
			global.trace('Action', {actionName: 'reallocating', roomName: room.name, creepName: creep.name, structureId: nuker.id, resourceType: RESOURCE_GHODIUM, needs: amount});
		if (room.storage && room.storage.store[RESOURCE_GHODIUM]) {
			if (global.DEBUG && global.TRACE)
				global.trace('Action', {actionName: 'reallocating', roomName: room.name, creepName: creep.name, targetStructureId: room.storage.id, resourceType: RESOURCE_GHODIUM, targetNeeds: room.storage.store[RESOURCE_GHODIUM]});
			creep.data.reallocating = RESOURCE_GHODIUM;
			return room.storage;
		}
		if (room.terminal && room.terminal.getNeeds(RESOURCE_GHODIUM) < 0) {
			if (global.DEBUG && global.TRACE)
				global.trace('Action', {actionName: 'reallocating', roomName: room.name, creepName: creep.name, targetStructureId: room.terminal.id, resourceType: RESOURCE_GHODIUM, targetNeeds: room.terminal.store[RESOURCE_GHODIUM]});
			creep.data.reallocating = RESOURCE_GHODIUM;
			return room.terminal;
		}
		...
		// if (ret) {
		// 	if (global.DEBUG && global.TRACE)
		// 		global.trace('Action', {actionName: 'reallocating', roomName: room.name, creepName: creep.name, targetStructureId: ret.structure.id, resourceType: RESOURCE_GHODIUM, targetNeeds: ret.amount});
		// 	creep.data.reallocating = RESOURCE_GHODIUM;
		// 	return ret.structure;
		// }

		// TODO it`s not working when global.ALLOCATE_COMPOUNDS is true
		// if (ROOM_TRADING && !global.MAKE_COMPOUNDS && global.ALLOCATE_COMPOUNDS && global.orderingRoom().length === 0) {
		//
		...
		// 			global.logSystem(room.name, `${creep.name} started a structure order of ${amount} ${RESOURCE_GHODIUM} for nuker ${nuker.id}`);
		// 		else
		// 			global.logSystem(room.name, `${creep.name} started a structure order of ${amount} ${RESOURCE_GHODIUM} for nuker ${nuker.id} IS: ${global.translateErrorCode(ret)}`);
		// 	}
		// 	// else {
		// 	//     room.updateRoomOrders();
		// 	//     let ret = room.fillARoomOrder()
		// 	//     if (global.DEBUG && ret === OK)
		// 	//         global.logSystem(room.name, `${creep.name} started a fillARoomOrder of ${amount} ${RESOURCE_GHODIUM} for nuker ${nuker.id}`);
		// 	//     else
		// 	//         global.logSystem(room.name, `${creep.name} started a fillARoomOrder of ${amount} ${RESOURCE_GHODIUM} for nuker ${nuker.id} IS ${global.translateErrorCode(ret)}`);
		// 	// }
		// }
	}
...
mod.newTarget = function (creep) {
	...
	if (creep.sum == 0) {
		...
		if (data) {
			...
			if (target === null) target = this.newTargetLab(creep);
...
mod.work = function (creep) {
	...
	if (creep.sum == 0) {
		...
		// if (room.name === 'E25S11') {
		// 	global.logSystem(room.name, `UNLOAD: creep: ${creep.name} target: ${target.structureType} mineral: ${global.json(creep.store)}`);
		// }
		...
		// if (room.name === 'E25S11') {
		// 	global.logSystem(room.name, `LOAD: creep: ${creep.name} target: ${target.structureType} mineral: ${global.json(creep.store)}`);
		// }
