...
mod.checkForRequiredCreeps = (flag) => {
	...
	// get task memory
	let memory = global.Task.sourceKiller.memory(flag);
	...
	if (count < 1) {
		...
		global.Task.spawn(
			...
			{ // spawn room selection params
				...
				minEnergyCapacity: Creep.bodyCosts(sourceKiller.fixedBody),
				maxRange: 3,
				allowTargetRoom: true,
			},
			creepSetup => { // callback onQueued
				let memory = global.Task.sourceKiller.memory(Game.flags[creepSetup.destiny.targetName]);
				memory.queued.push({
