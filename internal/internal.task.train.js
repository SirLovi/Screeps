...
mod.checkFlag = (flag) => {
	// 2x healer flag - flag.compareTo(FLAG_COLOR.trainHeal)
	if (flag.compareTo(global.FLAG_COLOR.trainTurret) || flag.compareTo(global.FLAG_COLOR.boostedTrain)) {
		global.Util.set(flag.memory, 'task', 'train');
		global.Util.set(flag.memory, 'type', 'trainTurret');
		global.Util.set(flag.memory, 'trainCount', 1);
		return true;
	}
	return false;
};
mod.handleFlagRemoved = flagName => {
	...
	if (flagMem && flagMem.task === 'train' && flagMem.roomName) {
		...
		let flags = global.FlagDir.filter((global.FLAG_COLOR.trainTurret), new RoomPosition(25, 25, flagMem.roomName), true);
		if (!(flags && flags.length > 0)) {
			for (let trainNum in global.Task.train.memory(flagMem)) {
				// no more train in that room, clear memory
				global.Task.removeQueued(global.Task.train.trainMemory(flagMem, trainNum));
			}
		}
	}
};
mod.handleFlagFound = flag => {
	// Analyze Flag
	if (global.Task.train.checkFlag(flag) && global.Task.nextCreepCheck(flag, mod.name)) {
		// check if a new creep has to be spawned
		global.Task.train.checkForRequiredCreeps(flag);
	}
};
...
mod.handleSpawningStarted = params => {
	...
	if (flag) {
		const memory = global.Task.train.trainMemory(flag, params.destiny.trainNum);
		if (params.body)
...
mod.handleSpawningCompleted = creep => {
	...
	if (flag) {
		...
		const memory = global.Task.train.trainMemory(flag, trainNum);
...
mod.handleCreepDied = name => {
	...
	if (flag) {
		...
		const memory = global.Task.train.trainMemory(flag, mem.destiny.trainNum);
		global.Task.validateRunning(memory, flag, mod.name, {roomName: mem.destiny.room, deadCreep: name});
	}
};
// check if a new creep has to be spawned
mod.checkForRequiredCreeps = (flag) => {
	...
	const members = global.Task.train.getMembers(flag);
	...
	for (let trainNum = 0; trainNum < trainCount; trainNum++) {
		...
		for (let order = 0; order < members.length; order++) {
			...
			if (!_.some([memory.running, memory.spawning, memory.queued], q => _.some(q, {trainOrder: order}))) {
				const definition = Object.assign({}, global.Task.train.creep[type]);
				definition.name = 'T' + trainNum + '.' + order + '-' + definition.name;
				let boosted = flag.compareTo(global.FLAG_COLOR.boostedTrain);
				...
				global.Task.spawn(
					...
					{ // spawn room selection params
						...
						// explicit: 'E25S18' // spawn train in that room only
					},
...
mod.getMembers = flag => {
	return global.Task.train.members[flag.memory.type];
};
mod.trainLength = type => {
	return global.Task.train.members[type].length;
};
...
mod.memory = flag => {
	return global.Util.get(flag.memory || flag, ['tasks', mod.name], {});
};
mod.trainMemory = (flag, trainNum) => {
	const memory = global.Util.get(global.Task.train.memory(flag), trainNum, {});
	global.Util.set(memory, 'queued', []);
	global.Util.set(memory, 'spawning', []);
	global.Util.set(memory, 'running', []);
	return memory;
};
mod.members = {
	trainTurret: [
