    ...
    let Structures = function (room) {
        ...
        Object.defineProperties(this, {
            ...
            'roads' : {
                ...
                get: function () {
                    if (_.isUndefined(this._labs)) {
                        this._labs = new Room.Roads(this.room);
                    }
                    return this._labs;
                }
            },
            'ramparts' : {
                ...
                get: function () {
                    if (_.isUndefined(this._labs)) {
                        this._labs = new Room.Ramparts(this.room);
                    }
                    return this._labs;
                }
            },
            'walls' : {
                ...
                get: function () {
                    if (_.isUndefined(this._labs)) {
                        this._labs = new Room.Walls(this.room);
                    }
                    return this._labs;
                }
            },
        ...
        Object.keys(global.COMPOUNDS_TO_MAKE).forEach(compound => {
            ...
            if (global.COMPOUNDS_TO_MAKE[compound].make && !roomFound && (global.COMPOUNDS_TO_MAKE[compound].rooms.indexOf(this.name) > -1 || global.COMPOUNDS_TO_MAKE[compound].rooms.length === 0)) {
                ...
                if (storedResources === 0) {
                    ...
                    roomFound = makeCompound(this.name, compound, amountToMake);
                    if (roomFound && global.DEBUG)
                        global.logSystem(this.name, `it is below the roomThreshold, so try to make the compounds for ${amountToMake} ${compound} in ${this.name}`);
                }
            }
        });
...
mod.shouldRepair = function (room, structure) {
    return (
        ...
        // not decayable or below roomThreshold
