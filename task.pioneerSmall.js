...
mod.handleFlagFound = flag => {
    ...
    if( flag.compareTo(FLAG_COLOR.claim.pioneerSmall) && Task.nextCreepCheck(flag, mod.name)){
        Util.set(flag.memory, 'task', mod.name);
        // check if a new creep has to be spawned
        Task.pioneerSmall.checkForRequiredCreeps(flag);
    }
};
// check if a new creep has to be spawned
mod.checkForRequiredCreeps = (flag) => {
    ...
    // get task memory
    let memory = Task.pioneerSmall.memory(flag);

    // re-validate if too much time has passed in the queue
    Task.validateAll(memory, flag, mod.name, {roomName: flag.pos.roomName, subKey: 'pioneerSmall', checkValid: true});
    ...
    if( count < 1 ) {
        const definition = Task.pioneerSmall.creep.pioneerSmall;
        Task.spawn(
            definition, // creepDefinition
            { // destiny
                task: 'pioneerSmall', // taskName
            ...
            creepSetup => { // callback onQueued
                let memory = Task.pioneerSmall.memory(Game.flags[creepSetup.destiny.targetName]);
                memory.queued.push({
...
mod.handleSpawningStarted = params => { // params: {spawn: spawn.name, name: creep.name, destiny: creep.destiny}
    ...
    if ( !params.destiny || !params.destiny.task || params.destiny.task != 'pioneerSmall' )
        return;
    ...
    if (flag) {
        ...
        const priority = type ? _.find(Task.pioneerSmall.creep, {behaviour: type}).queue : ['Low', 'High'];
...
mod.handleSpawningCompleted = creep => {
    ...
    if (!creep.data || !creep.data.destiny || !creep.data.destiny.task || creep.data.destiny.task != 'pioneerSmall')
        return;
...
mod.handleCreepDied = name => {
    ...
    // ensure it is a creep which has been requested by this task (else return)
    if (!mem || !mem.destiny || !mem.destiny.task || mem.destiny.task != 'pioneerSmall')
        return;
    // get flag which caused request of that creep
...
mod.memory = (flag) => {
    if( !flag.memory.tasks ) 
        flag.memory.tasks = {};
    if( !flag.memory.tasks.pioneerSmall ) {
        flag.memory.tasks.pioneerSmall = {
            queued: [], 
            spawning: [],
            running: []
        };
    }
    return flag.memory.tasks.pioneerSmall;
};
mod.creep = {
    pioneerSmall: {
        fixedBody: {
