	...
	defaultStrategy: {
		...
		spawnParams: function (flag) { //:{count:number, priority:string}
			...
			if (!hasFlag || !hasController) {
				if (DEBUG && TRACE) trace('Task', {
					hasFlag, hasController, checkForRequiredCreeps: 'skipping room, missing flag or controller',
			...
			if (flag.room) {
				...
				if (isOwned || validReservation) {
					if (DEBUG && TRACE) trace('Task', {
						validReservation, isOwned, checkForRequiredCreeps: 'skipping room, reserved or owned',
						[mod.name]: 'checkForRequiredCreeps', Task: mod.name,
					});
					return params;
				}
				...
				if (DEBUG && TRACE) {
					...
					trace('Task', {
						validReservation, isOwned, urgent, checkForRequiredCreeps: `sending${type}reserver`,
						[mod.name]: 'checkForRequiredCreeps', Task: mod.name,
					});
				}
