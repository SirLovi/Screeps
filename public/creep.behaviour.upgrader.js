...
mod.run = function (creep) {
	...
	if (creep.room.controller.upgradeBlocked && !creep.data.boosted) { // TODO boosting can happen in multiple passes!
		// TODO don't do this if you're boosted!
	...
	if (!creep.data.determinatedSpot) {
		...
		if (!creep.data.determinatedSpot) {
			global.logError('Unable to determine working location for upgrader in room ' + creep.pos.roomName);
		...
		} else if (global.SAY_ASSIGNMENT)
			creep.say(String.fromCharCode(9962), global.SAY_PUBLIC);
	}
	...
	if (creep.data.determinatedSpot) {
		...
		if (global.CHATTY)
			creep.say('upgrading', global.SAY_PUBLIC);
		let range = this.approach(creep);

		if (creep.room.controller && range <= 3) {
			...
			if (creep.carry.energy <= carryThreshold) {
				...
				if (!store)
					store = _.find(creep.room.structures.container.controller, s => s.store[RESOURCE_ENERGY] > 0 && creep.pos.isNearTo(s));
				if (!store) {
