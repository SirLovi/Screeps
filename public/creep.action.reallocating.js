...
//    if (!RESOURCES_ALL.find((r)=>{r==resourceType;})) return ERR_INVALID_ARGS;
	...
	if (storage && storage.active && (resourceType === RESOURCE_ENERGY || resourceType === RESOURCE_POWER) && storage.store.getCapacity() - storage.sum > amountMin) {
		return {structure: storage, amount: 0};
	}
	if (terminal && terminal.active && resourceType !== RESOURCE_ENERGY && resourceType !== RESOURCE_POWER && terminal.store.getCapacity() - terminal.sum > amountMin) {
	...
	if (data && data.labs && data.labs.length > 0) {
		for (let i = 0; i < data.labs.length; i++) {
			...
			if (lab.mineralAmount > 0) {
				...
				if (amount < 0) {
					...
					if (global.DEBUG && global.TRACE)
						global.trace('Action', {actionName: 'reallocating', roomName: room.name, creepName: creep.name, structureId: lab.id, resourceType: lab.mineralType, needs: amount});
					needing = this.findNeeding(room, lab.mineralType);
					if (needing) {
						if (global.DEBUG && global.TRACE)
							global.trace('Action', {actionName: 'reallocating', roomName: room.name, creepName: creep.name, targetStructureId: needing.structure.id, resourceType: lab.mineralType, targetNeeds: needing.amount});
						return lab;
					}
				}
				if (amount > 0) {
					...
					if (ret) {
						if (global.DEBUG && global.TRACE)
							global.trace('Action', {actionName: 'reallocating', roomName: room.name, creepName: creep.name, targetStructureId: ret.structure.id, resourceType: lab.mineralType, targetNeeds: ret.amount});
						creep.data.reallocating = lab.mineralType;
						return ret.structure;
					}
					if (global.ROOM_TRADING && !(global.MAKE_COMPOUNDS || global.ALLOCATE_COMPOUNDS) && !(lab.mineralType === RESOURCE_ENERGY || lab.mineralType === room.mineralType)) {
				...
				if (order) {
					...
					if (global.DEBUG && global.TRACE)
						global.trace('Action', {actionName: 'reallocating', roomName: room.name, creepName: creep.name, structureId: lab.id, resourceType: resourceType, needs: amount});
					if (room.storage && room.storage.active && room.storage.store[resourceType]) {
						if (global.DEBUG && global.TRACE) global.trace('Action', {
							actionName: 'reallocating',
							roomName: room.name,
							creepName: creep.name,
							targetStructureId: room.storage.id,
					...
					if (ret) {
						if (global.DEBUG && global.TRACE)
							global.trace('Action', {actionName: 'reallocating', roomName: room.name, creepName: creep.name, targetStructureId: ret.structure.id, resourceType: resourceType, targetNeeds: ret.amount});
						creep.data.reallocating = resourceType;
						return ret.structure;
					}
					if (global.ROOM_TRADING && !(global.MAKE_COMPOUNDS || global.ALLOCATE_COMPOUNDS) && !(resourceType === RESOURCE_ENERGY || resourceType === room.mineralType)) {
						const orderCreated = _.some(room.memory.resources.orders, {'type': lab.mineralType});
						if (!orderCreated)
			...
			if (amount < 0) {
				...
				if (global.DEBUG && global.TRACE)
					global.trace('Action', {actionName: 'reallocating', roomName: room.name, creepName: creep.name, structureId: lab.id, resourceType: RESOURCE_ENERGY, needs: amount});
				let needing = this.findNeeding(room, RESOURCE_ENERGY);
				if (needing) {
					if (global.DEBUG && global.TRACE)
						global.trace('Action', {actionName: 'reallocating', roomName: room.name, creepName: creep.name, targetStructureId: needing.structure.id, resourceType: RESOURCE_ENERGY, targetNeeds: needing.amount});
					return lab;
				}
			}
			if (amount > 0) {
				...
				if (ret) {
					if (global.DEBUG && global.TRACE)
						global.trace('Action', {actionName: 'reallocating', roomName: room.name, creepName: creep.name, targetStructureId: ret.structure.id, resourceType: RESOURCE_ENERGY, targetNeeds: ret.amount});
					creep.data.reallocating = RESOURCE_ENERGY;
					return ret.structure;
				}
			}
	...
	if (powerSpawns.length > 0) {
		for (let i = 0; i < powerSpawns.length; i++) {
			...
			if (amount > 0) {
				...
				if (global.DEBUG && global.TRACE) global.trace('Action', {actionName: 'reallocating', roomName: room.name, creepName: creep.name, structureId: powerSpawn.id, resourceType: RESOURCE_ENERGY, needs: amount});
				if (room.storage && room.storage.active && room.storage.charge > 0.5) {
					if (global.DEBUG && global.TRACE) global.trace('Action', {
						actionName: 'reallocating',
						roomName: room.name,
			...
			if (amount > 0) {
				// powerSpawn needs energy so find a lower priority container with some
				if (global.DEBUG && global.TRACE) global.trace('Action', {actionName: 'reallocating', roomName: room.name, creepName: creep.name, structureId: powerSpawn.id, resourceType: RESOURCE_POWER, needs: amount});
				...
				if (ret) {
					if (global.DEBUG && global.TRACE)
						global.trace('Action', {actionName: 'reallocating', roomName: room.name, creepName: creep.name, targetStructureId: ret.structure.id, resourceType: RESOURCE_POWER, targetNeeds: ret.amount});
					creep.data.reallocating = RESOURCE_POWER;
					return ret.structure;
				}
			}
			...
			if (container) {
				...
				for (let resource in container.store) {
					...
					if (resource && needs < 0) {
						...
						if (global.DEBUG && global.TRACE)
							global.trace('Action', {actionName: 'reallocating', roomName: room.name, creepName: creep.name, structureId: container.id, resourceType: resource, needs: needs});
						let needing = this.findNeeding(room, resource);
						if (needing) {
							if (global.DEBUG && global.TRACE)
								global.trace('Action', {actionName: 'reallocating', roomName: room.name, creepName: creep.name, targetStructureId: needing.structure.id, resourceType: resource, targetNeeds: needing.amount});
					...
					if (containerData) {
						...
						for (let j = 0; j < orders.length; j++) {
							...
							if (amount > 0) {
								...
								if (global.DEBUG && global.TRACE) global.trace('Action', {actionName: 'reallocating', roomName: room.name, creepName: creep.name, structureId: container.id, resourceType: resource, needs: amount});
...
//                    if (resource && (resource == RESOURCE_ENERGY || resource == RESOURCE_POWER)) {
			...
			if (amount > 0) {
				...
				if (global.DEBUG && global.TRACE) global.trace('Action', {actionName: 'reallocating', roomName: room.name, creepName: creep.name, structureId: terminal.id, resourceType: resource, needs: -amount});
				let dest = this.findNeeding(room, resource, 1, terminal.id);
				if (dest && dest.structure.id !== terminal.id) {
					if (global.DEBUG && global.TRACE) global.trace('Action', {actionName: 'reallocating', roomName: room.name, creepName: creep.name, targetStructureId: dest.structure.id, resourceType: resource, targetNeeds: dest.amount});
					creep.data.reallocating = resource;
					return terminal;
				}
			}
//                    }
		...
		if (room.memory.resources && room.memory.resources.terminal && room.memory.resources.terminal[0]) {
			...
			for (let i = 0; i < orders.length; i++) {
				...
				if (amount > 0) {
					// found a needed resource so check lower priority containers
					if (global.DEBUG && global.TRACE) global.trace('Action', {actionName: 'reallocating', roomName: room.name, creepName: creep.name, structureId: terminal.id, resourceType: type, needs: amount});
					if (room.storage && room.storage.active && room.storage.store[type] && !(type === RESOURCE_ENERGY && room.storage.charge < 0.5)) {
						if (global.DEBUG && global.TRACE) global.trace('Action', {
							actionName: 'reallocating',
							roomName: room.name,
	...
	if (storage && storage.active) {
		...
		for (let resource in storage.store) {
			...
			if (resource && amount > 0) {
				if (global.DEBUG && global.TRACE) global.trace('Action', {actionName: 'reallocating', roomName: room.name, creepName: creep.name, structureId: storage.id, resourceType: resource, needs: -amount});
...
action.assignDropOff = function (creep, resource) {
	...
	if (data) {
		if (global.DEBUG && global.TRACE)
			global.trace('Action', {actionName: 'reallocating', roomName: creep.room.name, creepName: creep.name, subAction: 'assignDropOff', targetStructureId: data.structure.id, resourceType: resource, amount: data.amount});
...
action.unloadLab = function (creep) {
	...
	// TODO if (target !== seedLAb) -> get next nonSeedLabs with resource to target, check if creeps not full, this.unloadLab(creep)

	// if(workResult === OK && creep.getCapacity() - creep.sum > 0) {
	//
	// }
	...
	for (let i = 0; i < store.length; i++) {
		...
		if (res && target.store[res] > 0 && (target.structureType === STRUCTURE_LAB || target.getNeeds(res) < 0)) {
			...
			if (amount > 0) {
				...
				amount = storage.store.getCapacity() - storage.sum;
				resource = res;
				break;
			}
		}
...
action.unloadTerminal = function (creep) {
	...
	// console.log(`REALLOCATING TERMINAL:`);
	let store = Object.keys(target.store);
	if (creep.data.reallocating) {
		store.unshift(creep.data.reallocating);

		// global.logSystem(creep.room.name, `target: ${target}`);
		// global.logSystem(creep.room.name, `reAllocate: ${creep.data.reallocating}`);
		// global.logSystem(creep.room.name, `store: ${global.json(store)}`);
	...
	for (let i = 0; i < store.length; i++) {
		...
		let needs = target.getNeeds(res);
		// if (needs !== 0)
		// 	global.logSystem(creep.room.name, `creep: ${creep.name} target: ${target} mineral: ${res} getNeeds: ${needs}`);
	...
	if (resource) {
		...
		workResult = this.unloadStructure(creep, target, resource, amount);
		// global.logSystem(creep.room.name, `TERMINAL UNLOAD resource: ${resource} ERR: ${global.translateErrorCode(workResult)}`);
	}
	if (global.DEBUG && global.TRACE)
		global.trace('Action', {actionName: 'reallocating-unloadTerminal', roomName: room.name, creepName: creep.name, structureId: target.id, resourceType: resource, needs: amount, workResult});
	if (workResult === OK) {
		this.assignDropOff(creep, resource);
	} else
...
action.unloadStorage = function (creep) {
	...
	// console.log(`REALLOCATING STORAGE:`);
	...
	for (let i = 0; i < store.length; i++) {
		...
		if (res && target.store[res] > 0) {
			if (res === RESOURCE_ENERGY && target.charge < 0.5)
				continue;
			let dat = this.findNeeding(room, res, 1, target.id);
			if (global.DEBUG && global.TRACE)
				global.trace('Action', {actionName: 'reallocating', subAction: 'findNeeding', roomName: room.name, creepName: creep.name, structureId: dat && dat.structure.id, resourceType: res, amount: dat && dat.amount});
			if (dat && dat.structure.id === target.id)
				dat = null;
			if (dat) {
				amount = dat.amount;
			}
			...
			if (amount > 0) {
				...
				amount = terminal.store.getCapacity() - terminal.sum;
				resource = res;
				break;
			}
		}
	...
	if (resource) {
		amount = Math.min(amount, target.store[resource] || 0, creep.carryCapacity - creep.sum);
		workResult = this.unloadStructure(creep, target, resource, amount);
		// global.logSystem(creep.room.name, `STORAGE UNLOAD resource: ${resource} ERR: ${global.translateErrorCode(workResult)}`);
	}
	if (global.DEBUG && global.TRACE)
		global.trace('Action', {actionName: 'reallocating-unloadStorage', roomName: room.name, creepName: creep.name, structureId: target.id, resourceType: resource, needs: amount, workResult});
	if (workResult === OK) {
		this.assignDropOff(creep, resource);
	} else this.cancelAction(creep);
	return workResult;
...
action.loadTerminal = function (creep) {
	...
	for (let res in creep.carry) {
		...
		if (amount > 0) {
			...
			resource = res;
			amount = target.store.getCapacity() - target.sum;
			break;
		}
	}
	if (resource)
		workResult = this.loadStructure(creep, target, resource, amount);
