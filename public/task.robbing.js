...
mod.checkFlag = (flag) => {
    ...
    return flag.compareTo(global.FLAG_COLOR.invade.robbing) && global.Task.nextCreepCheck(flag, mod.name);
};
// for each flag
mod.handleFlagFound = flag => {
    // if it is a robbing flag
    if (global.Task.robbing.checkFlag(flag)) {
        global.Util.set(flag.memory, 'task', mod.name);
        // check if a new creep has to be spawned
        global.Task.robbing.checkForRequiredCreeps(flag);
    }
};
// check if a new creep has to be spawned
mod.checkForRequiredCreeps = (flag) => {
    // get task memory
    let memory = global.Task.robbing.memory(flag);
    ...
    // if creep count below requirement spawn a new creep
    if (count < (memory.numRobbers || 2)) {
        ...
        const storageRoom = global.ROBBER_REHOME ? spawnRoom : mod.strategies.robber.homeRoom(flag);
        global.Task.spawn(
            global.Task.robbing.creep.robbing, // creepDefinition
            { // destiny
            ...
            creepSetup => { // callback onQueued
                let memory = global.Task.robbing.memory(Game.flags[creepSetup.destiny.targetName]);
                memory.queued.push({
...
mod.handleCreepDied = name => {
    ...
    if (flag) {
        const memory = global.Task.robbing.memory(flag);
    ...
    if (creep.ticksToLive > 1350 && creep.data && !creep.data.boosted) {
        ...
        if (labs.length > 0) {
            ...
            if (global.SAY_ASSIGNMENT)
                creep.say(String.fromCharCode(9883), global.SAY_PUBLIC);
        ...
        if (carrySum > 0) {
            ...
            if (deposit.length > 0) {
                ...
                if (target.structureType === STRUCTURE_STORAGE && this.assignAction(creep, 'storing', target))
                    return;
                else if (Creep.action.charging.assign(creep, target))
                    return;
            } else if (Creep.action.dropping.assign(creep)) {
                return;
            } else {
                //if( Creep.action.storing.assign(creep) ) return;
                if (Creep.action.charging.assign(creep))
                    return;
    ...
    // not at home
        ...
        if (creep.data.destiny.targetName && Game.flags[creep.data.destiny.targetName].pos.roomName === creep.pos.roomName) {
            ...
            // carrier full
                ...
                global.Population.registerCreepFlag(creep, null);
...
mod.getFlag = function (roomName) {
    ...
    return global.FlagDir.find(global.FLAG_COLOR.invade.robbing, Game.rooms[roomName]);
};
mod.storage = function (roomName, storageRoom) {
    const memory = global.Task.robbing.memory(mod.getFlag(roomName));
    if (storageRoom) {
