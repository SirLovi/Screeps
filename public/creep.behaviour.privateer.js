...
mod.run = function (creep) {
	...
	if (global.DEBUG && global.TRACE)
		global.trace('Behaviour', {creepName: creep.name, run: creep.action && creep.action.name || 'none', [this.name]: 'run', Behaviour: this.name});
};
	...
	if (creep.pos.roomName === creep.data.homeRoom) {
		...
		if (carrySum > 0) {
			...
			if (this.assignAction(creep, 'charging'))
				return;
			if (!creep.room.ally && this.assignAction(creep, 'storing'))
				return;
