...
mod.extend = function () {
	...
	Object.defineProperty(StructureStorage.prototype, 'charge', { // fraction indicating charge % relative to constants
		...
		get: function () {
			...
			return Util.chargeScale(this.store.energy,
				global.MIN_STORAGE_ENERGY[this.room.controller.level],
				global.MAX_STORAGE_ENERGY[this.room.controller.level]);
		},
	});
	StructureStorage.prototype.getNeeds = function (resourceType) {
		...
		let unloadTarget = (resourceType == RESOURCE_ENERGY) ? (this.store.getCapacity() - this.sum) + this.store.energy : order.orderAmount + order.storeAmount + MAX_STORAGE_MINERAL;
		if (unloadTarget < 0) unloadTarget = 0;
		let store = this.store[resourceType] || 0;
		if (store < loadTarget) ret = Math.min(loadTarget - store, this.store.getCapacity() - this.sum);
	...
	StructureContainer.prototype.getNeeds = function (resourceType) {
		...
		if (containerData) {
			...
			if (order) {
				...
				if (unloadTarget < 0) unloadTarget = 0;
				let store = this.store[resourceType] || 0;
				if (store < loadTarget) return Math.min(loadTarget - store, this.store.getCapacity() - this.sum);
	...
	StructureLab.prototype.getNeeds = function (resourceType) {
		...
		if (containerData) {
			...
			if (order) {
				...
				else if (resourceType === this.mineralType)
					amt = this.store[this.mineralType];
		...
		if (resourceType === RESOURCE_ENERGY) {
			...
			space = this.store.getCapacity() - store;
			cap = this.store.getCapacity(RESOURCE_ENERGY);
		} else {
			if (this.mineralType === resourceType)
				store = this.store[this.mineralType];
			space = this.store.getCapacity(this.mineralType) - store;
			cap = this.store.getCapacity(this.mineralType || resourceType);
		}
		...
		if ((store < Math.min(loadTarget, cap) / 2) || store < LAB_REACTION_AMOUNT)
